...
function handleGlobalClick(e) {
    ...
    if (downloadBtn) {
        e.stopPropagation();
        handleDownloadReport(downloadBtn, downloadBtn.dataset.reportId);
        return;
    }
    ...
    if (editUserBtn) {
        openUserModal(editUserBtn.dataset.userId);
        return;
    }
...
async function renderAllReportsView() {
    ...
    try {
        ...
        container.innerHTML = `<table class="data-table">
            ...
            ${reports.map(report => `
                <tr data-report-id="${report.id}" title="Click to view details">
                    ...
                    <td>${report.ai_priority || 'N/A'}</td>
                    <td><span class="status-badge status-${report.status.toLowerCase().replace(' ', '-')}">${report.status}</span></td>
                    <td>${report.citizen.email}</td>
                    <td>${report.assigned_technician?.profile?.full_name || report.assigned_technician?.email || 'Unassigned'}</td>
                    <td>
                        <div class="action-buttons">
                            ...
                            <button class="btn btn-sm btn-download-report" data-report-id="${report.id}">Download</button>
                        </div>
                    </td>
        ...
        document.getElementById('reports-table-container').innerHTML = `<p class="error-message">Error loading reports: ${error.message}</p>`;
    }
}
...
async function renderReportDetailView(reportId) {
    ...
    try {
        ...
        const citizenImage = report.image ? `<div class="media-item"><img src="${report.image}" alt="Citizen Submission" class="thumbnail" data-full-src="${report.image}"><span class="media-label">Citizen</span></div>` : '';
        const technicianImages = report.report_updates.filter(update => update.image).map(update => `<div class="media-item"><img src="${update.image}" alt="Technician Update" class="thumbnail" data-full-src="${update.image}"><span class="media-label">Technician</span></div>`).join('');
        const updatesHTML = report.report_updates.map(update => `<div class="timeline-item"><div class="timeline-content"><p class="remark-text">"${update.remark}"</p><span class="timestamp">by ${update.technician?.profile?.full_name || 'System'} on ${new Date(update.created_at).toLocaleString()}</span></div></div>`).join('');
        const technicianOptions = technicians.map(tech => `<option value="${tech.id}" ${report.assigned_technician?.id === tech.id ? 'selected' : ''}>${tech.profile.full_name} (${tech.email})</option>`).join('');
            ...
            <div class="detail-grid">
                <div class="detail-main">
                    ...
                    <div class="card">
                        ...
                        <div class="media-gallery">${citizenImage || technicianImages ? citizenImage + technicianImages : '<p>No images submitted.</p>'}</div>
                    </div>
                ...
                <div class="detail-sidebar">
                    <div class="card manage-report-card">
                        ...
                        <div class="detail-section">
                            <h4>Status</h4>
                            <div class="status-update-form"><select id="detail-status-select">${statusOptions}</select><button class="btn btn-sm btn-update-detail-status" data-report-id="${report.id}">Update</button></div>
                        </div>
                         <div class="detail-section"><h4>Current Priority</h4><p>${report.ai_priority || 'Not Set'}</p></div>
                    ...
                    <div class="card technician-assignment-card">
                        ...
                        <div class="detail-section">
                            ...
                            <p class="assigned-tech-name">${report.assigned_technician?.profile?.full_name || report.assigned_technician?.email || 'Unassigned'}</p>
                        </div>
                        <div class="technician-search"><input type="search" id="technician-search-input" placeholder="Search by name or email..."><button class="btn btn-sm" id="technician-search-btn">Search</button></div>
                        <div class="assign-group"><select id="technician-select" class="technician-select">${technicianOptions}</select><button id="assign-technician-btn" data-report-id="${report.id}" class="btn btn-sm">Assign</button></div>
                    </div>
                    <div class="card">
...
async function renderTechniciansView() {
    mainContentContainer.innerHTML = `
        <div class="page-actions">
            <button class="btn btn-create-user">Create New Technician</button>
        </div>
        <div class="card">
            <div id="users-table-container">Loading technicians...</div>
        </div>
    `;
    try {
        const technicians = await api.getAllTechnicians();
        adminState.users = technicians;
        ...
        container.innerHTML = `<table class="data-table">
            ...
            `).join('') || `<tr><td colspan="6">No technicians found.</td></tr>`}
            </tbody>
        </table>`;
    } catch (error) {
        document.getElementById('users-table-container').innerHTML = `<p class="error-message">Error loading technicians: ${error.message}</p>`;
    }
}
...
async function openUserModal(userId) {
    ...
    const modalTitle = isEditing ? `Edit Technician #${user.id}` : 'Create New Technician';
    const passwordFieldHtml = !isEditing ? `<div class="input-group"><label for="modal-password">Password</label><input type="password" id="modal-password" required autocomplete="new-password"></div>` : '';
            ...
            <div class="modal-content" style="max-width: 500px;">
                ...
                <form id="user-form">
                    <div class="modal-body">
                        <div class="input-group"><label for="modal-fullname">Full Name</label><input type="text" id="modal-fullname" value="${user?.profile.full_name || ''}" required></div>
                        <div class="input-group"><label for="modal-email">Email</label><input type="email" id="modal-email" value="${user?.email || ''}" required></div>
                        ${passwordFieldHtml}
                        <div class="input-group"><label for="modal-phone">Phone Number</label><input type="tel" id="modal-phone" value="${user?.profile.phone_number || ''}" required></div>
                        <div class="input-group">
                            <label for="modal-gender">Gender</label>
                            <select id="modal-gender" required>
                                <option value="" disabled ${!user ? 'selected' : ''}>Select gender</option>
                        ...
                        <div class="input-group"><label for="modal-specialization">Specialization</label><input type="text" id="modal-specialization" value="${user?.profile.specialization || ''}"></div>
                    </div>
...
// --- CORRECTED LOGIC TO PREVENT SERVER CRASH ---
async function handleSaveUser(userId) {
    ...
    try {
        let response;
        if (isEditing) {
            // For PATCH updates, we build the profile data carefully
            const profileData = {
            ...
            response = await api.updateInternalUser(userId, updateData);
        } else {
            // For POST creation, we send a payload that exactly matches the /auth/register/ endpoint
            const createData = {
                email: document.getElementById('modal-email').value,
                ...
                profile: {
                    ...
                    phone_number: document.getElementById('modal-phone').value,
                    role: 'technician',
                    // Note: gender & specialization are intentionally omitted here to match the 
                    // backend's simple RegisterSerializer and prevent a crash.
                    // These can be added later by editing the technician.
                }
            };
            response = await api.createInternalUser(createData);
        }

        closeModal();
        alert(`Technician ${isEditing ? 'updated' : 'created'} successfully!`);
        adminState.users = null; // Invalidate cache
        renderTechniciansView(); // Refresh the view
    } catch (error) {
...
async function handleDeleteUser(userId) {
    ...
    if (!user) return;
    if (confirm(`Are you sure you want to delete technician ${user.profile.full_name}? This action cannot be undone.`)) {
        try {
...
async function openEditReportModal(reportId) {
    ...
    const modalHTML = `<div class="modal-overlay"><div class="modal-content"><div class="modal-header"><h2>Edit Report #${report.id}</h2><button class="modal-close">&times;</button></div><div class="modal-body"><div class="input-group"><label for="modal-category">Category</label><select id="modal-category">${categoryOptions}</select></div><div class="input-group"><label for="modal-description">Description</label><textarea id="modal-description" rows="4">${report.description}</textarea></div><div class="input-group"><label for="modal-status">Status</label><select id="modal-status">${statusOptions}</select></div><div class="input-group"><label for="modal-priority">Priority</label><select id="modal-priority">${priorityOptions}</select></div></div><div class="modal-footer"><button class="btn modal-cancel-btn">Cancel</button><button class="btn modal-save-btn" data-report-id="${report.id}">Save Changes</button></div></div></div>`;
    document.body.insertAdjacentHTML('beforeend', modalHTML);
}
...
async function handleTechnicianSearch() {
    ...
    try {
        ...
        if (technicians.length === 0) {
            ...
            technicianSelect.innerHTML = technicians.map(tech => `<option value="${tech.id}">${tech.profile.full_name} (${tech.email})</option>`).join('');
        }
    } catch (error) {
...
function openImageModal(imageUrl) {
    const modalHTML = `<div class="image-modal-overlay"><span class="image-modal-close">&times;</span><img src="${imageUrl}" alt="Full size view" class="image-modal-content"></div>`;
    document.body.insertAdjacentHTML('beforeend', modalHTML);
...
async function handleDownloadReport(button, reportId) {
